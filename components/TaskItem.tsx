...
export function TaskItem({ item, listId }: Props) {
  ...
  const toggleMutation = useMutation({
    ...
    onMutate: async (completed) => {
      ...
      const prev = qc.getQueryData<Item[]>(["task-items", listId]);
      qc.setQueryData<Item[]>(["task-items", listId], (old) =>
    ...
    >
      <button
        ...
        className={`w-6 h-6 rounded-full border-2 flex-shrink-0 flex items-center justify-center transition-colors ${
          item.completed ? "bg-amber-500 border-amber-500" : "border-stone-300 hover:border-amber-400"
        }`}
